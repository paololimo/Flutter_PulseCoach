...
| `E19R-1-CB` run the two-peer smoke on a schedule/CI (creds-gated) | Not run in 21.5/22, **but the blocker changed:** a CI pipeline now exists (`E22R-1` → `.github/workflows/ci.yml`) and the smoke test exists (`pulse_coach/test/integration/shared_session_two_peer_smoke_test.dart`) | **Wired this session (2026-07-07) — pending secrets** | Added `.github/workflows/two-peer-smoke.yml`: nightly `schedule` + `workflow_dispatch`, gated to the canonical repo, runs the smoke with `--dart-define` from repo secrets. Verified locally that the test self-skips when defines are absent (skip, not red), so a missing secret degrades gracefully. **Remaining human step (Paolo):** set repo secrets `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SMOKE_PASSWORD` in GitHub → Settings → Secrets. Until then the job runs but skips. |
...
**Action item `E22R-3` (Category A — deliverable) — OPENED AND CLOSED same session (2026-07-07):** re-materialized the five project-specific review traps into a **sync-durable, git-tracked home**: a new `## Project-Specific Code-Review Traps` section in `_bmad-output/project-context.md`. This home is durable because (a) it lives under `{project-root}`, not `{skill-root}`, so the MCPmarket baseline sync (which only overwrites `{skill-root}/customize.toml` and `SKILL.md`) cannot touch it; (b) it is git-tracked (history + cross-machine); (c) it is **already auto-loaded** by `bmad-code-review` — the default `workflow.persistent_facts` glob `file:{project-root}/**/project-context.md` resolves to it (verified via `resolve_customization.py`), and the code-review workflow also loads `**/project-context.md` directly (SKILL.md step). The Edge-Case Hunter receives the traps through the review's `also_consider` seam. Traps homed: (1) `Hero`-in-`AnimatedSwitcher` collision [E7-P3], (2) Cubit lifecycle invariants [E8-P1], (3) scrollable shimmer [E18R-CB1], (4) localized-IT on failure paths [E18R-CB2], (5) two-client render comparison [E20R-B1]. `E18R-CB1/CB2/E20R-B1/E7-P3/E8-P1` are now **enforced**, not tracked-but-unenforced.
