  ...
  group('GetSensorContext', () {
    ...
    test('3.3-UNIT-001: both sources succeed → all fields populated', () async {
      when(mockGetHealthData()).thenAnswer(
        (_) async => const Right(HealthData(restingHr: 62, stepCount: 4500)),
      );
      when(
        mockGetActivityLevel(),
    ...
    test(
      ...
      () async {
        ...
        when(
          mockGetActivityLevel(),
        ).thenAnswer((_) async => const Right(ActivityLevel.sedentary));
    ...
    test(
      ...
      () async {
        when(mockGetHealthData()).thenAnswer(
          (_) async => const Right(HealthData(restingHr: 58, stepCount: 8200)),
        );
        when(mockGetActivityLevel()).thenAnswer(
          (_) async => const Left(SensorFailure('Accelerometer unavailable')),
        );
    ...
    test(
      ...
      () async {
        ...
        when(
          mockGetActivityLevel(),
        ).thenAnswer((_) async => const Right(ActivityLevel.active));
