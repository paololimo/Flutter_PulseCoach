  ...
  group('WeatherRepositoryImpl.getWeatherContext', () {
    ...
    test(
      ...
      () async {
        ...
        when(
          mockRemote.fetchWeatherAndAqi(latitude: 48.8, longitude: 2.3),
    ...
    test(
      ...
      () async {
        final freshCachedAt = DateTime.now().toUtc().subtract(
    ...
    test(
      ...
      () async {
        final staleCachedAt = DateTime.now().toUtc().subtract(
        ...
        when(
          mockLocal.getCachedWeather(),
        ).thenAnswer((_) async => staleContext);
        when(
          mockRemote.fetchWeatherAndAqi(latitude: 48.8, longitude: 2.3),
        ).thenAnswer((_) async => (weatherModel, aqiModel));
        when(
    ...
    test(
      ...
      () async {
        ...
        final exactlyAtBoundary = DateTime.now().toUtc().subtract(
        ...
        result.fold((_) => fail('Expected Right'), (context) {
          expect(
            context.temperature,
            20.0,
          ); // fresh data fetched, not stale 15.0
        });
        verify(
          mockRemote.fetchWeatherAndAqi(latitude: 48.8, longitude: 2.3),
        ).called(1);
